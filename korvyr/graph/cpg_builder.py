...
#: Optional CSV of CPG build failures, enabled by setting
#: ``KORVYR_CPG_FAILURE_LOG`` to a writable path. Off by default so scanning a
#: package never writes into the caller's working tree.
CPG_FAILURE_LOG_ENV = "KORVYR_CPG_FAILURE_LOG"
...
def _record_cpg_failure(
    ...
    """Append a CPG build failure row when failure logging is enabled."""
    configured = os.getenv(CPG_FAILURE_LOG_ENV, "").strip()
    if not configured:
        return
    failures_path = Path(configured)
    try:
...
def _build_metadata_tensor(
    ...
    pj = read_package_json(package_dir)
